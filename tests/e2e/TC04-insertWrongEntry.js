...
describe('An wrong entry is created', function () {
	it('Should deny the new entry', function (){
		...
		element(by.model('contact.name')).sendKeys('');
		element(by.model('contact.email')).sendKeys('test@mail.com');
		...
		var new_contacts = element.all(by.repeater('contact in contactlist')).count().then(function(new_contacts_def) {
			...
			//Hacer el assert
			expect(numContactsEnd).toEqual(numContactsStart);
		});
